            ...
            <div class="table-container">
                ...
                <div class="table-scroll">
                    <table>
                        <thead>
                            <tr>
                                <th><input type="checkbox" id="select_all_id"></th>
                                <th>#</th>
                                <th>Kompetisi</th>
                                <th>Nama Atlet</th>
                                <th>Nomor Lomba</th>
                                    ...
                                    <tr>
                                        <td>
                                            <input type="checkbox" name="ids" class="checkbox_ids"
                                                   value="{{ $index }}"
                                                   data-harga="{{ $peserta['harga'] }}"
                                                   data-group="{{ $index }}">
                                        </td>
                                        <td>{{ $loop->iteration }}</td>
                                        <td>{{ $peserta['kompetisi'] }}</td>
                                        <td>{{ $peserta['nama'] }}</td>
                                        <td>
                                            <ul>
                                                @foreach ($peserta['pesertas'] as $detail)
                                                    <li style="display: flex; align-items:center;">
                                                        <input type="checkbox" name="detail_ids[]"
                                                               class="checkbox_detail"
                                                               value="{{ $detail->id }}"
                                                               data-group="{{ $index }}"
                                                               style="display: none;">
                                                        <p>{{ $detail->getAcara->nomor_lomba }} - {{ $detail->getAcara->nama }}</p>
                                                        <div class="actions">
                                                            <form action="{{ route('dashboard.tagihan.destroy', $detail->id) }}" method="post">
                                                                @csrf
                                                                @method('delete')
                                                                <a onclick="return confirm('Apakah kamu yakin ingin menghapus? ')">
                                                                    <button class="button-red button-gap" data-tooltip="Hapus Tagihan">
                                        ...
                                        <td>
                                            <span class="status bayar">
                                                Rp{{ number_format($peserta['harga'], 2, ',', '.') }}
                                            </span>
                                        </td>
                                    </tr>
                                @endforeach
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    selectAllCheckbox.addEventListener("click", function() {
        mainCheckboxes.forEach(mainCheckbox => {
            ...
            const relatedCheckboxes = document.querySelectorAll(`.checkbox_detail[data-group="${index}"]`);
    ...
    function updateTotalHarga() {
        ...
        totalHargaElement.textContent = "Rp" + total.toLocaleString("id-ID", { minimumFractionDigits: 2 });

        if (totalAtlet > 0) {
            payButton.style.display = "block";
            payButton.textContent = `Bayar (${totalAtlet})`;
        } else {
